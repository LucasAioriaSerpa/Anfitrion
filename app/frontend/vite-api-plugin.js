  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        // 4. Auth: Stats
        if (pathname === '/api/auth/stats' || pathname === '/api/auth/estatisticas') {
          const totalHospedes = users.filter((u) => u.role === 'hospede').length;
        ...
        if (pathname === '/api/hospede') {
          if (method === 'GET') {
            return sendJson(res, 200, users.filter((u) => u.role === 'hospede'));
          }
          if (method === 'POST') {
            const body = await readBody(req);
            const newHosp = { id_hospede: Date.now(), ...body, role: 'hospede' };
            users.push(newHosp);
        ...
        if (pathname === '/api/funcionario') {
          return sendJson(res, 200, users.filter((u) => u.role === 'funcionario'));
        }
